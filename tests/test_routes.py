...
class TestProductRoutes(TestCase):
    ...
    def test_update_product_not_found(self):
        """It should return 404 if the product to update does not exist"""
        # on ne crée pas de produit du tout
        fake_id = 9999
        product_data = {
            "name": "Ghost",
        ...
        response = self.client.put(f"{BASE_URL}/{fake_id}", json=product_data)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        data = response.get_json()
    ...
    def test_method_not_supported(self):
        ...
        # Try to PUT on collection URL (not allowed)
